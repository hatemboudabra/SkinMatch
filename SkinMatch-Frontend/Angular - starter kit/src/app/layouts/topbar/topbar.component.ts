...
@Component({
  selector: 'app-topbar',
  standalone: true,
  imports: [DrawerModule, MnDropdownComponent, LucideAngularModule, SimplebarAngularModule, RouterModule, RouterLink,CommonModule],
...
export class TopbarComponent {
  ...
  private loadCurrentUser() {
    ...
    if (storedId && /^\d+$/.test(storedId)) {
      this.fetchAndBindUser(storedId);
      return;
    }
    ...
    this.userService.getUsers().subscribe({
      next: (users) => {
        ...
        if (match && (match as any).id != null) {
          ...
          this.fetchAndBindUser(numericId);
        }
      }
    });
  }

  private fetchAndBindUser(id: string) {
    this.userService.getUserById(id).subscribe({
      next: (user) => {
        ...
        this.userPhotoUrl = this.userService.getPhotoUrl(user.photo);
      },
      error: () => {
